#'Analise de experimento fatorial duplo com testemunhas no DBC
#'@description Esta funcao considera experimentos em esquema fatorial no
#'  delineamento em blocos casualizados.Esta funcao retorna a comparacao
...
#'  considers experiments in factorial scheme for the randomized block design.
#'  This function returns the multiple comparison tests ( t, t with Bonferroni
#'  protection, Duncan, Dunnet, SNK, Tukey and Scott-Knott) if the factor is
...
#'  identificacao das repeticoes (Blocos). Na quinta coluna  os resultados da
#'  variavel resposta.
#'
#'@param Protegido Valor logico. Se for FALSE os testes de medias serao feitos
#'  indendente das significancias pelo teste F na ANOVA.Se for TRUE os testes de
#'  medias serao feitas apenas quando haver efeito significativo pelo teste F.
...
#'
#' data(Dados3)
#' fatorial2.ad.dbc(Dados3,Protegido=FALSE,alfa = 0.05,quali = c(TRUE,TRUE))
#'
#' ##Exemplo de experimento com uma testemunha adicional e um fator qualitativo
#' data(Dados4)
#' fatorial2.ad.dbc(Dados4,Protegido=TRUE,alfa = 0.05,quali = c(FALSE,TRUE))
#'
#' ##Exemplo de experimento com tres testemunhas adicionais e um fator qualitativo
#' data(Dados5)
#' fatorial2.ad.dbc(Dados5,Protegido=TRUE,alfa = 0.05,quali = c(TRUE,FALSE))
#'@export
...
sq=function(Fac,x){
  ...
  y=tapply(as.numeric(x),as.factor(Fac),mean,na=T)
  r=mean(table(Fac))
  SQ=sum((y-mean(y))^2)*r
  QM=SQ/GL
...
Residuo=Total-colSums(rbind(Parte1,Parte2,Bloco))
Residuo[3]=Residuo[2]/Residuo[1]

ANOVA=data.frame(rbind(Parte1,Parte2,Bloco,Residuo,Total))
...
if(verbose){print(ComparacaoMedias((var[id]), as.character(Dados[id,2]),
...
if(quali[1]==F){
 ...
 # ComparacaoMedias((var[id]), as.character(Dados[id,2]), Residuo[1], Residuo[3], alpha = alfa, group = TRUE,main = NULL)
  Reg=RegressaoPolinomial((var[id]), as.numeric(as.character((Dados[id,2]))),
                        glres=Residuo[1], Residuo[2], FatorA[1], FatorA[2])
  if(verbose){print("______________________________________________________")}
}
...
if(verbose){print(ComparacaoMedias((var[id]), as.character(Dados[id,3]),
                       Residuo[1], Residuo[3], alpha = alfa, group = TRUE,main = NULL))}
if(verbose){print("______________________________________________________")}
}
if(quali[2]==F){
 ...
 # ComparacaoMedias((var[id]), as.character(Dados[id,3]), Residuo[1], Residuo[3], alpha = alfa, group = TRUE,main = NULL)
  reg=RegressaoPolinomial((var[id]), as.numeric(as.character(Dados[id,3])), glres=Residuo[1], SQres=Residuo[2], gltrat =FatorB[1], SQtrat=FatorB[2])
  if(verbose){print("______________________________________________________")}
}
...
for(i in (unique(as.character(Dados[id,3])))){
  if(verbose){print(paste("Desdobramento dos niveis de A dentro do nivel",i,"do fator B"))}
   id3=(id*(Dados[,3]==i))==1
   ...
   if(quali[1]==F){
     ...
     Reg=RegressaoPolinomial(Dados[id3,5], as.numeric(as.character((Dados[id3,2]))), glres=Residuo[1], Residuo[2], A.B[1], A.B[2])
...
for(i in (unique(as.character(Dados[id,2])))){
  ...
  if(quali[2]==F){
    ...
    Reg=RegressaoPolinomial(Dados[id3,5], as.numeric(as.character(Dados[id3,3])), glres=Residuo[1], Residuo[2],  A.B[1], A.B[2])
